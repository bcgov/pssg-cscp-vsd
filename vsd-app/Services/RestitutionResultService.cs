...
{
    public interface IRestitutionResultService
    {
        Task<DynamicsResult> Get(string endpointUrl);
        Task<DynamicsResult> Post(string endpointUrl, string requestJson);
    }

    public class RestitutionResultService : IRestitutionResultService
    {
        private HttpClient _client;
        private IConfiguration _configuration;
        private readonly ILogger _logger;

        public RestitutionResultService(IConfiguration configuration, HttpClient httpClient)
        {
            _client = httpClient;
        ...
        {
            DynamicsResult blob = await RestitutionResultAsync(HttpMethod.Get, endpointUrl, "");
            return blob;
        }

        public async Task<DynamicsResult> Post(string endpointUrl, string modelJson)
        {
            DynamicsResult blob = await RestitutionResultAsync(HttpMethod.Post, endpointUrl, modelJson);
            return blob;
        }

        private async Task<DynamicsResult> RestitutionResultAsync(HttpMethod method, string endpointUrl, string requestJson)
        {
