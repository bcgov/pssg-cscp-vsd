...
export class RepresentativeInformationComponent extends FormBase implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.methodOfContactSubscription = this.form
      .get('representativePreferredMethodOfContact')
    ...
    this.addressInfoSubscription = this.form
      .get('mostRecentMailingAddressSameAsPersonal')
      .valueChanges.subscribe((value) => {
  ...
  setRequiredFields(completingOnBehalfOf: number) {
    ...
    if (completingOnBehalfOf === 100000003) {
      //legal rep, fill in relationshipToPerson
    ...
    if (useValidation) {
      ...
      this.setControlValidators(representativePreferredMethodOfContact, [
        Validators.required,
        Validators.min(100000000),
        Validators.max(100000002)
      ]);
    } else {
  ...
  setRepresentativePhoneValidators() {
    ...
    if (
      this.form.get('representativeAddress.country').value === 'Canada' ||
      this.form.get('representativeAddress.country').value === 'United States of America'
    ) {
      phoneMinLength = 10;
    } else {
      phoneMinLength = 8;
