...
export class MedicalInformationComponent extends FormBase implements OnInit, OnDestroy {
  ...
  setDoctorPhoneValidators() {
    if (
      this.form.get('familyDoctorAddress.country').value === 'Canada' ||
      this.form.get('familyDoctorAddress.country').value === 'United States of America'
    ) {
  ...
  setProviderPhoneValidators(provider: AbstractControl) {
    if (
      provider.get('providerAddress.country').value === 'Canada' ||
      provider.get('providerAddress.country').value === 'United States of America'
    ) {
      this.phoneMinLength = 10;
    } else {
      this.phoneMinLength = 8;
    ...
    this.setControlValidators(faxControl, [
      Validators.minLength(this.phoneMinLength),
      Validators.maxLength(this.phoneMaxLength)
    ]);
    phoneControl.patchValue(phoneControl.value);
